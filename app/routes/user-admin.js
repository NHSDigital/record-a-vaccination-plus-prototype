module.exports = (router) => {
  ...
  // add-user-mvp
  router.post('/user-admin/add-user-mvp', function (req, res) {
    const choice = req.session.data['clinicmanager']; 
    // "radio-choice" must match the name attribute of your radio input

    if (choice === 'yes') {
      res.redirect('/user-admin/add-pathways-mvp?mvp=true')
    } else if (choice === 'no') {
      res.redirect('/user-admin/check?mvp=true')
    } else {
