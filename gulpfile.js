...
// Copy ONLY the patient data JS file(s) without transpiling
function copyPatientData() {
  return gulp
    .src('app/assets/javascript/send-invite-data/**/*.js', { allowEmpty: true })
    .pipe(gulp.dest('public/js/send-invite-data'))
}
...
function watch() {
  gulp.watch('app/assets/javascript/send-invite-data/**/*.js', copyPatientData)
...
exports.copyPatientData = copyPatientData

gulp.task(
  'build',
  gulp.series(cleanPublic, compileStyles, compileScripts, compileAssets, copyPatientData)
)
